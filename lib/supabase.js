...
export async function getColleges(filters = {}) {
  let query = supabase
    .from('colleges')
    .select(`
      id, slug, name, city, state, nirf_rank, tier, color, emoji,
      avg_package, cat_cutoff, tags, verdict,
      work_exp_required, work_exp_preferred, tagline, founded,
      programs(id, total_fees, slug)
    `)
  ...
  return (data || []).map(c => ({
    ...c,
    min_fees:           c.programs?.length > 0
                          ? Math.min(...c.programs.map(p => p.total_fees || Infinity).filter(f => f !== Infinity))
                          : null,
    program_count:      c.programs?.length || 0,
    avg_package:        c.avg_package        || null,
    cat_cutoff:         c.cat_cutoff         || null,
    tags:               c.tags               || [],
    verdict:            c.verdict            || null,
    work_exp_required:  c.work_exp_required  || false,
    work_exp_preferred: c.work_exp_preferred || false,
  }))
}
