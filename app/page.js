...
const FEATURES=[{icon:'⚖️',title:'Side-by-side comparisons',body:'Compare any two colleges on fees, placements, cutoffs, culture. AI gives a verdict for your specific goals.',href:'/compare'},{icon:'🧠',title:'CAT readiness test',body:'Take 3 real CAT questions. AI predicts your percentile and shows exactly what to improve.',href:'#minicat-section'},{icon:'📊',title:'Verified placement data',body:'Every number shows when verified and from which source. Confidence score on every data point.',href:'/colleges'},{icon:'🎯',title:'Eligibility checker',body:'Enter your full profile. AI calculates your realistic match % for each college with reasons.',href:'/eligibility'},{icon:'📅',title:'Deadline tracker',body:'AI tracks all upcoming application windows, PI dates, and shortlist releases. Auto-updated.',href:null},{icon:'📖',title:'Expert MBA guides',body:'IIM comparisons, fee ROI, CAT strategy, city guides — written by AI, verified by data.',href:'/blog'}]
    ...
    <div style={{minHeight:'100vh'}}>
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'48px 32px 64px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:14,marginBottom:48}}>
          {FEATURES.map((f,i)=>(
            <div key={i} style={{background:'var(--white)',border:'1px solid var(--border)',borderRadius:12,padding:20,cursor:'pointer',transition:'all .2s'}}
              onClick={()=>f.href&&(f.href.startsWith('#')?document.querySelector(f.href)?.scrollIntoView({behavior:'smooth'}):null)}
          ...
          {DEADLINES.map((d,i)=>(
            <div key={i} style={{background:'var(--white)',border:'1px solid var(--border)',borderRadius:11,padding:16,display:'flex',gap:14,alignItems:'flex-start',transition:'transform .2s'}}
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:10,fontFamily:'var(--mono)',marginTop:6,color:urgColor[d.urgency],fontWeight:d.urgency==='urgent'?500:'normal'}}>{d.days} days remaining</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{background:'var(--ink)',borderRadius:16,padding:32,textAlign:'center'}}>
          <div style={{fontFamily:'var(--serif)',fontSize:'1.3rem',fontWeight:700,color:'#fff',marginBottom:8}}>150+ MBA guides — free</div>
      ...
      <footer style={{background:'#060504',borderTop:'1px solid rgba(255,255,255,.06)',padding:'32px',textAlign:'center'}}>
        ...
        <div style={{display:'flex',justifyContent:'center',gap:20,fontSize:12,flexWrap:'wrap'}}>
          {[['Colleges','/colleges'],['Compare','/compare'],['Eligibility','/eligibility'],['Blog','/blog']].map(([l,h])=>(
